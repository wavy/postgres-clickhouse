...
docker-compose exec clickhouse clickhouse-client -q 'CREATE TABLE default.demo_table (`id` Int64, `data` Nullable(Int8)) ENGINE = MergeTree() PRIMARY KEY (id);'
docker-compose exec clickhouse clickhouse-client -q "INSERT INTO default.demo_table (id, data) VALUES (0, 127);" 
...
docker-compose exec pg psql -U postgres -c "INSERT INTO public.demo_table (id, data) VALUES (2, -128)"
docker-compose exec clickhouse clickhouse-client -q "SELECT * FROM default.demo_table ORDER BY id;"
docker-compose exec pg psql -U postgres -c "SELECT * FROM public.demo_table ORDER BY id;"
